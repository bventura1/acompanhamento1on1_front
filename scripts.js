...
const filter = () => {
  let inputColaborador = document.getElementById("colaborador").value;

  if (inputColaborador === '') {
    alert("Escreva o nome de um colaborador!");
  }
  //  else if (isNaN(inputColaborador) || isNaN(inputStatus)) {
  //   alert("Quantidade e valor precisam ser números!");
  // } 
    ...
    const getListFilter = async (inputColaborador = null) => {
      let url = 'http://192.168.0.40:5000/atividades';
      if (inputColaborador) {
        url += `?colaborador=${inputColaborador}`;
      }
      fetch(url, {
        method: 'get',
      })
        .then((response) => response.json())
        .then((data) => {
                // Limpar todas as linhas existentes da tabela
          document.getElementById('myTable').innerHTML = '';
          data.forEach(item => insertList(item.tarefa, item.colaborador, item.status));
        })
        .catch((error) => {
          console.error('Error:', error);
        });
